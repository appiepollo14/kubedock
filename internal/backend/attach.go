...
func (in *instance) AttachContainer(c *types.Container, stdin io.Reader, stdout io.Writer, stderr io.Writer, tty bool) error {
	pod, err := in.cli.CoreV1().Pods(in.namespace).Get(context.Background(), c.GetPodName(), v1.GetOptions{})
	if err != nil {
		return err
	}
	...
	req := attach.Request{
		...
		RestConfig: in.cfg,
		Pod:        *pod,
		Container:  "main", // same convention as ExecContainer
		TTY:        tty,
	}
